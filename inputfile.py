...
class InputFile(object):
	...
	def __init__(self, **kwargs):
		...
		self.commands = {'FILE':'file','NEW':'new','RENAME':'rename','REMOVE':'remove', 'COLOR':'color', 'SIZE':'size', 'EVALUE':'evalue', 'DB':'db', 'SETUP':'setup'}
	...
	def setupRemote(self,subcontent):
		...
		if len(data) >0:
			...
			print "HMM database name is not found. Remote Hmmscan will be perfomed using pfam",format(data[0])
			self.db = "pfam"
			self.local = False
		return
	...
	def setup(self,content):
		subcommands= {'LOCAL':'setupLocal', 'REMOTE' : 'setupRemote', 'THRESHOLD' : 'setupThreshold', 'EVALUE' : 'setupEvalue', 'OUTPUTHTML':'setupOutputHTML', 'OUTPUTSVG':'setupOutputSVG'}
