...
class miscAnnotation(object):
	...
	def	annotationReader(self):
		if self.sequence and len(self.sequence)>0:
			...
			print "Running {0} prediction of {1} using web service..".format(self.method, self.file)
			...
			request = urllib2.Request('http://hmmer.janelia.org/annotation/{0}'.format(self.method),enc_params)
			#get the url where the results can be fetched from
			results_url=urllib2.urlopen(request).headers.dict['location']
			...
			self.name = "Disorder Prediction:"+self.name
			...
			if result:
				...
				if match:
					...
					if 'motifs' in data:
						...
						for motif in motifs:
							desc = 'feature'
					 		hit = HmmerHit(name=self.method, desc=desc,start=motif['start'], end=motif['end'])
					 		hit.border = False
					 		hit.startshow = False
					 		hit.endshow = False
					 		hit.label = False
...
if __name__ == "__main__":
	...
	miscFeatures['coils']
	drawer = SVGDrawer(outputSVG=item+'_disorder.svg', hmmerResults = miscResults)
	drawer.drawSVG()
