	...
	def secondaryDraw(self):
		...
		while (cursor<self.length):
			...
			for i in range(len(predict)):
				...
				prediction = ET.Element('text', x=str(leftMargin+i*fontSize*0.625), y=str(y+fontSize), fill=color, 
									style='font-family:Courier;font-size:'+str(fontSize)+'px;text-anchor:left;dominant-baseline:middle')							
				prediction.text = predictText[predict[i]]				
				doc.append(prediction)

			if (cursor+columnWidth < self.length):
				endnumbers = ET.Element('text', x=str(leftMargin+columnWidth*fontSize*0.68), y=str(y), fill='black', 
									style='font-family:Courier;font-size:'+str(fontSize)+'px;text-anchor:end;dominant-baseline:middle')
				endnumbers.text = str(int(self.number[cursor])+columnWidth-1)
		...
		aminoacids = ET.Element('text', x=str(leftMargin), y=str(y), fill='black', 
								style='font-family:Courier;font-size:'+str(fontSize)+'px;text-anchor:left;dominant-baseline:middle')
		aminoacids.text = text
		prediction = ET.Element('text', x=str(leftMargin), y=str(y+fontSize), fill='black', 
								style='font-family:Courier;font-size:'+str(fontSize)+'px;text-anchor:left;dominant-baseline:middle')
